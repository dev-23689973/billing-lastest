...
import { getManagerSubscriberActivityByDayRange } from "@/lib/repos/managerDashboard";
...
export async function GET(req: Request) {
  ...
  try {
    const days = await getManagerSubscriberActivityByDayRange(s.username, from, to);
    return apiJson({ days });
  } catch {
