...
import { DEALER_SUBSCRIBERS_PORTAL } from "@/lib/subscribersPortalTable";
...
export default async function DealerUsersPage({ searchParams }: Props) {
  ...
  if (!session || session.type !== "RSLR") redirect("/login?error=forbidden");
  ...
  const [summary, cfg, expiringSoon, { rows, total }, tariffs, customPlanId] = await Promise.all([
    timeServerLoad("dealer-users:summary", () =>
      getUsersSummaryScoped({ ownerType: "RSLR", ownerUsername: session.username }),
    ),
    timeServerLoad("dealer-users:deductions", () => getDeductionsConfig()),
    timeServerLoad("dealer-users:expiring-soon", () =>
      getScopedExpiringSoonCount({ ownerType: "RSLR", ownerUsername: session.username, withinDays: 7 }).catch(() => 0),
    ),
    timeServerLoad("dealer-users:listAccountsPaged", () => listAccountsPagedScoped(listParams)),
    timeServerLoad("dealer-users:tariffs", () => listStalkerTariffPlans()),
    timeServerLoad("dealer-users:custom-plan-id", () => getStalkerCustomPackagePlanId()),
  ]);
  const addonPackages =
    customPlanId != null && Number.isFinite(customPlanId) && customPlanId > 0
      ? await timeServerLoad("dealer-users:addon-packages", () => listStalkerPackagesForPlan(customPlanId))
      : [];
    ...
    ...(sp.error === "save"
      ...
      : sp.error === "pin"
        ...
        : sp.error === "packages"
          ...
          : []),
    ...newEndUserCreationFlashItems(sp, "dealer"),
    ...(sp.ok === "reset"
      ? [
          {
  ...
  return (
    <div className="flex h-full min-h-0 w-full min-w-0 flex-1 flex-col overflow-hidden">
      ...
      <PageHeader title="Users" breadcrumb="Manage and monitor your subscriber accounts." showBack={false} />
      ...
      <section className="flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-hidden border-t border-border/40">
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
          ...
          subscribersPortal={DEALER_SUBSCRIBERS_PORTAL}
          actionLinks={{
            exportFilters,
            addSubscriberHref: "/dealer/users/new",
            canAddSubscriber,
          }}
          embedded
          toolbarFilters={{
