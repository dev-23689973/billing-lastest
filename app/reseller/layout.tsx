...
export default async function ResellerLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <BillingAnnouncementGate session={session} shellExtras={shellExtras}>
      ...
      >
        <AdminSidebar session={session} portalBase="/reseller" panelTitle={panelTitle} />
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden lg:ml-60">
          <AdminAppHeader
            ...
            portalBase="/reseller"
            notificationsHref="/reseller/tickets/dashboard"
            ticketPreview={ticketPreview}
          />
          <AppMain className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden overflow-y-auto">{children}</AppMain>
        </div>
      </div>
