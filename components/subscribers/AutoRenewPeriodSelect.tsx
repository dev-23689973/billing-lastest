...
export function AutoRenewPeriodSelect({
  value,
  onValueChange,
  options,
  labelledBy,
  ...
  const reposition = useCallback(() => {
    ...
    const top = openUp ? Math.max(VIEW_MARGIN, r.top - ANCHOR_GAP - Math.min(maxHeight, 320)) : r.bottom + ANCHOR_GAP;
        ...
        <div className={cn(managersToolbarSearchableDropdownListBoxClass, "flex min-h-0 flex-1 flex-col overflow-hidden")}>
          ...
          >
            {options.map((o) => {
              ...
              const isDisable = o.value === "disable";
              return (
                <button
                  key={o.value}
                  type="button"
                  ...
                  className={cn(
                    managersToolbarSearchableDropdownItemCompactClass,
                    "!justify-start gap-2",
                    selectedRow && "bg-violet-500/10 font-medium",
                    isDisable && "text-muted-foreground",
                  )}
                  role="option"
                ...
                >
                  <span className="inline-flex h-4 w-4 shrink-0 items-center justify-center">
                    {selectedRow ? (
                      ...
                      <Repeat className="h-3.5 w-3.5 text-muted-foreground/80" aria-hidden />
                    )}
                  </span>
                  <span className="min-w-0 flex-1 truncate text-left text-sm">{o.label}</span>
                </button>
              );
            })}
          </div>
    ...
    <div ref={rootRef} className="relative min-w-0">
      <button
        ...
        className={cn(
          ...
          "h-10 border-violet-500/35 px-3 focus-visible:border-violet-500 focus-visible:ring-violet-500/20",
        ...
        aria-expanded={open}
      >
        <span className="min-w-0 flex-1 truncate text-left text-sm leading-none">{selected?.label ?? "Select…"}</span>
