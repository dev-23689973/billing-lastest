/**
 * STB send_msg KPI — layered 3D rings + dropdown belts.
 */
...
import {
  buildStalkerChartLayers,
  buildStalkerMessageBeltRows,
  STALKER_CHART_LAYER_COLORS,
  type StalkerChartLayer,
} from "@/lib/messages/stalkerMessageKpiRows";
...
function layerArcAngles(layer: StalkerChartLayer): { start: number; end: number } | null {
...
function StalkerLayeredRingScene3D({
  stats,
  isLight,
}: {
  ...
  const layers = useMemo(() => buildStalkerChartLayers(stats), [stats]);
      ...
      <group rotation={[0.34, 0, 0]}>
        <group ref={groupRef} position={[0, 0.02, 0]} scale={SCENE_SCALE}>
          {layers.map((layer) => {
            ...
            if (!arc) return null;
            const c = STALKER_CHART_LAYER_COLORS[layer.key];
            return (
              <ExtrudedSector
                key={layer.key}
                innerR={layer.innerR}
...
export function StalkerMessageStatusArcs({
  stats,
  subscriberReach,
  className,
  mobileCompact = false,
  ...
  const rows = useMemo(() => buildStalkerMessageBeltRows(stats, subscriberReach), [stats, subscriberReach]);
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`STB messaging: ${formatInt(total)} rows, ${formatInt(delivered)} delivered, ${formatInt(Math.max(0, total - delivered))} queued.`}
    >
      {mobileCompact ? (
      ...
      >
        ...
        <div className="relative z-[1] pointer-events-none flex h-full w-full items-center justify-center p-0">
          {canvasReady ? (
            ...
            >
              <StalkerLayeredRingScene3D stats={stats} isLight={isLight} />
            </Canvas>
          ) : (
