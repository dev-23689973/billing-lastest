...
import { resetManagerStaffPasswordAction, resetResellerStaffPasswordAction, resetStaffPasswordAction } from "@/actions/forms";
...
import { staffDetailsCloseButtonClass, staffDetailsStatTileClass, staffDetailsStatValueClass } from "@/components/admin/managers-toolbar-icon-button";
...
export function AdminDealerRowActions({
  ...
  staffPortal?: "admin" | "manager" | "reseller";
}) {
  const isManagerPortal = staffPortal === "manager";
  const isResellerPortal = staffPortal === "reseller";
  const editorApiBase = isResellerPortal ? "/api/reseller" : isManagerPortal ? "/api/manager" : "/api/admin";
  const passwordResetAction = isResellerPortal
    ? resetResellerStaffPasswordAction
    : isManagerPortal
      ? resetManagerStaffPasswordAction
  ...
  return (
    <div className="relative flex justify-center">
      ...
      <FloatingMenuPortal open={open} onOpenChange={setOpen} anchorRef={anchorRef}>
        ...
        <div className={floatingRowActionMenuDividerClass} onClick={(e) => e.stopPropagation()}>
          {canDelete ? (
            ...
            <ManagerDeleteDealerForm username={username} canDelete={false} buttonLabel="Delete dealer" menuItem />
          ) : isResellerPortal ? (
            <ResellerDeleteDealerForm username={username} canDelete={false} buttonLabel="Delete dealer" menuItem />
          ) : (
          ...
          <header className="mb-4 flex items-start justify-between gap-3 border-b border-border/60 pb-4 dark:border-cyan-400/10">
            ...
            <div className="mb-3 grid grid-cols-2 gap-2">
              ...
              <div className={staffDetailsStatTileClass}>
                ...
                <p className={staffDetailsStatValueClass}>{managerLogin || "—"}</p>
              </div>
              <div className={cn(staffDetailsStatTileClass, "col-span-2")}>
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Reseller</p>
                <p className={staffDetailsStatValueClass}>{resellerLogin || "—"}</p>
              </div>
            </div>
      ...
      <StaffPasswordResetModal
        ...
        idPrefix={`dealer-reset-${username}`}
        redirectPath={redirectPath}
      />
      <StaffTransactionsOverlay
        ...
        transactionsApiBase={editorApiBase}
      />
      <AdminStaffEditModalTrigger
        rowType="DEALER"
        username={username}
        editorApiBase={editorApiBase}
