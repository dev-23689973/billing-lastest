...
/** Confirm saving an inline-edited subscriber field (name, MAC, password). */
export function ConfirmFieldUpdateModal({
  entityName,
  saving,
  onCancel,
  onConfirm,
  titleId = "field-update-confirm-title",
  zIndexClass = "z-[320]",
}: {
  entityName: string;
  saving: boolean;
      ...
      >
        ...
        <div className="relative z-[1] p-5 sm:p-6">
          ...
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
            Save changes for <span className="font-medium text-foreground">{entityName}</span>?
          </p>
          ...
          <div className="mt-6 flex items-center justify-end gap-2.5 border-t border-cyan-600/15 pt-4 dark:border-t-cyan-400/10">
            ...
            <button
              ...
              onClick={onConfirm}
              autoFocus
              className="inline-flex h-9 items-center rounded-md bg-primary px-3.5 text-sm font-semibold text-primary-foreground shadow-sm transition-colors hover:bg-primary/90 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/40 disabled:cursor-not-allowed disabled:opacity-60"
            >
